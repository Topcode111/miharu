...
function set_limit($uid, $id, $data) {
    ...
    if (! empty($lastset)) {
        ...
        //優先順位３より低いものは入力される毎バージョンアップ
           switch ($pr) {
            case 4:
                dbmgr::upvdbslavesetlog4($id,$pr,$onoff,$time,$ctrl);
                $res = '00';
                return $res;
            case 5:
                dbmgr::upvdbslavesetlog5($id,$pr,$onoff,$time,$ctrl);
                $res = '00';
                return $res;
            case 6: 
            ...
            //優先順位３より高い物 
            //時間設定をすべてOFFにする。
            dbmgr::upenddbslavesetlogoff($id);
        ...
        switch ($pr) {
            ...
            case 3:
                ...
                return $res;
            case 4:
                dbmgr::writedbslavesetlog4($id, $pr, $onoff, $time, $ctrl);
                $res = '00';
                return $res;
            case 5: 
                dbmgr::writedbslavesetlog5($id, $pr, $onoff, $time, $ctrl);
                $res = '00';
                return $res;
            case 6:
...
//子機のゲート情報取得
function get_batch($id, $uid) {
    $order = "";
    $data = dbmgr::readdbgw($id);
    if (count($data) !== 0 ) {
        $arraylist = $data[0];
        $order = implode(",", $arraylist);
    }
...
function set_batch($id, $uid, $values) {
    $values = preg_split('/,/', $value);
    dbmgr::write_batch($id, $uid, $values);
    return "00";
}
